...
def create_author():
    ...
    return make_response({f"Author {new_author.name} sucessfully created", 201})
...
def validate_author(author_id):
    try: 
        ...
        abort(make_response({"msg":f"author {author_id} invaild"}, 400))

    author = Author.query.get(author_id)

    if not author:
        abort(make_response({"msg":f"author {author_id} not found"}, 404))
